...
function VoidWanderers:MissionCreate()
	...
	self.missionData["nextGoldWarning"] = self.missionData["enemyBudget"] + (self.missionData["targetGold"] - self.missionData["enemyBudget"]) * 0.20;
	...
	count = math.max(1, math.ceil(#miners * self.missionData["spawnRate"] * 2));
	...
	count = math.max(1, math.ceil(#security * self.missionData["spawnRate"]));
	...
	count = math.max(1, math.ceil(#snipers * self.missionData["spawnRate"] / 2));
	snipers = CF.RandomSampleOfList(snipers, count);

	-- Spawn miners
	for i = 1, #miners do
		self:SpawnViaTable{
			Preset = CF.PresetTypes.ENGINEER,
			Team = CF.CPUTeam,
			Player = self.missionData["missionTarget"],
			AIMode = Actor.AIMODE_GOLDDIG,
			Pos = miners[i],
		};
	end
	...
	for i = 1, #security do
		self:SpawnViaTable{
			Preset = math.random(CF.PresetTypes.HEAVY2),
			Team = CF.CPUTeam,
			Player = self.missionData["missionTarget"],
			AIMode = Actor.AIMODE_SENTRY,
			Pos = security[i],
		};
	end
	...
	if self.missionData["stage"] == CF.MissionStages.ACTIVE then
		...
		if enemyFunds > 0 then
			...
			if enemyFunds > self.missionData["nextGoldWarning"] then
				...
				self.missionData["lastFailWarning"] = tonumber(self.GS["Time"]) + 5;
			end
			...
			if tonumber(self.GS["Time"]) < self.missionData["lastFailWarning"] then
				for player = Activity.PLAYER_1, Activity.MAXPLAYERCOUNT - 1 do
					FrameMan:ClearScreenText(player);
					local text = "STOP ENEMY MINING OPERATION\n"
							.. self.missionData["targetGold"] - math.ceil(enemyFunds)
							.. "oz OF GOLD LEFT TO MINE";
					FrameMan:SetScreenText(text, player, 0, 1000, true);
				end
			end
		...
		for actor in MovableMan.Actors do
			if actor.Team == CF.CPUTeam then
				...
				if actor.ClassName == "ACDropShip" or actor.ClassName == "ACRocket" then
					if not SceneMan:IsUnseen(actor.Pos.X, actor.Pos.Y, CF.PlayerTeam) then
						self:AddObjectivePoint("TAKE DOWN\nDROP SHIP", actor.AboveHUDPos, CF.PlayerTeam, GameActivity.ARROWDOWN);
					end
